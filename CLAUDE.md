...
| v1.27 Витрина (storefront) | Shipped 2026-06-30 (merge `4bc08b4`, задача #3 funnel-roadmap закрыта): `/dashboard` → редакторская **витрина** — лента полок на пассивном онбординг-сигнале (`UserProfile.goals/marketplaces`) + ручные теги. **Хребет:** `badges String[]` на `Lesson`+`Job` (аддит. миграция `20260629030000_add_content_badges` via Mgmt API). Таксономия `START`/`NEW`/`HOT`/`QUICK` (ручные, сид `seed-content-badges.ts`; первая пачка 12ур+6джоб на проде). **Полки** (`dashboard.getStorefront`): Начни отсюда/Продолжить/Под задачу:{goal}/Новое на {МП}/Быстрые/Хит — **state-aware порядок** (возвращающийся→Продолжить сверху, START скрыт). **Карусель** `Shelf.tsx` (умные стрелки) → «Смотреть все» → **страница-коллекция** `/dashboard/collection/[shelfKey]` (`getCollection`, группировка Задачи/Уроки + тогл + чипы). **База знаний:** вернули `FilterPanel` (отвалился в 61-05) + фильтр по тегам. **Различение Задача/Урок (вариант B):** `JobCard`=индиго-тонировка+кант+пилюля «Задача»; `LessonCard`=белый+пилюля «Урок»; верхняя строка `[тип][бейджи]`. **Верх сжат:** hero −дубль-CTA, 4 цветные кнопки в ряд, убрана дубль-карточка «Продолжить урок». Затянул origin/master +10 коммитов (PR #19–22) чисто. ГОЧИ: карусель уравняла ширину→различать тонировкой не формой; `getCourses` ронял `badges`→блокер фильтра (фикс 1 строкой, поймал сквозной ревью); HOT по `completedAt` не row-count. api 335/ai 82/web 324/typecheck 6/6. Откат `git revert -m 1 4bc08b4`. Память `project_storefront_dashboard.md`) |
| v1.28 Диагностика по осям | Shipped 2026-07-03 (merge `1397a93`): ось-центричный редизайн диагностики/плана — единый ось-хребет (радар=результаты=план) вместо абстрактных «уровней», `AxisLearningPath v3` (секции по 5 осям, капы 5/ось+20 общий, relevance-подбор, migrate-on-read), экран результатов один поток (блок «Как устроено обучение» + задачи с «Закрывает: {ось}—N%» + capped teaser + CTA→`/learn/plan`, стена трёх списков убрана), план `/learn/plan` ось-аккордеон. **Разбор ошибок ожил** — статик-дек Phase 59 не нёс привязку вопрос→урок; прогнали RAG (`scripts/diagnostic/map-questions-to-lessons.ts`) → **29/30 вопросов** привязаны к урокам-ответкам в `static-deck.ts` (q-wb-04 без), флаг «⚠ Разбор ошибки» going-forward. Paywall-CTA урока (`LockOverlay`/`PaywallBanner`)→`/billing`. Аддитивно, без миграций, `LessonProgress` не тронут. api 366/shared 4/typecheck 0/web 362+1 флейк. Откат `git revert -m 1 1397a93`. Память `project_diagnostic_axis_redesign.md`) |
| v1.29 Пост-онбординг первый урок | Shipped 2026-07-08 (merge `f9120da`): развилка визарда удалена → новый юзер падает на `/dashboard` с директивным героем «Твой первый урок» (градиент), урок подобран `resolveFirstLesson(goals,marketplaces)` по мэппингу 7целей×2МП на реальные lessonId (методологи, `docs/first-lesson-shortlist-2026-07-07.md`); «Начни отсюда» пересобран визард-driven (`resolveGoalLessons` + добор из оси, не фикс-`START`); диагностика снята со входа (осталась закрываемая `DiagnosticGateBanner` в уроках); визуал дашборда (приветствие без фона, порядок кнопок под новичка, «Найти быстрый ответ»→pink). Жизненный цикл героя = по касанию. Аддитивно, без миграций. Гоча: модалка завершения только при `?from=` контексте (голый hero-вход → авто-переход). api 403/typecheck 6/6/web 8/8 welcome. Откат `git revert -m 1 f9120da`. Память `project_post_onboarding_first_lesson.md`) |
| v1.28.1 Trial gap fix | Shipped 2026-07-08 (merge `3b5e6f1`): `/auth/callback` не звал `ensureBaseTrial` → ~46% подтверждённых без авто-триала (paywall как награда за онбординг). Зеркалил реф/триал-блок из `/auth/confirm`, гейт на recovery. Бэкафилл 57 триалов (created≥29.06, confirmed, no-sub). Гоча: legacy `service_role` JWT отключён (утечка 2026-05-21) → Admin API новым `sb_secret_*`. Откат `git revert -m 1 3b5e6f1`) |
| v1.30 Вход через Точку | Shipped **LIVE** 2026-07-08 (merge `fce6470` #28 + scope-fix `7e55a9b` #29): «Войти через Точку» (Tochka ID OAuth) на `/login`+`/register`, зеркалит Яндекс-флоу, OAuth-юзер создаётся `email_confirm:true` → обходит недоставку DOI на `@tochka.com`. OIDC-клиент портирован из go.mpstats (Variti-WAF-обход). Флаг `TOCHKA_LOGIN_ENABLED`, аддит. `UserProfile.tochkaId` (Mgmt API). **Прод-находка:** боевой клиент `245da9…` принимает ТОЛЬКО `scope=default` (openid/customers→400 «Incorrect scope»); `default` **отдаёт email** (проверено). E2E owner: линк-ветка ✅ (zebrosha@list.ru), ветка нового юзера — mock+Yandex-parity. Логотип — фиолетовый бейдж «Точка Банк». Откат: флаг→false + `up -d`. Память `project_tochka_oauth_login_shipped.md`) |
...
## Last Session (2026-07-08) — «Вход через Точку» (Tochka ID OAuth) shipped LIVE to prod

**merge `fce6470` (#28) + scope-fix `7e55a9b` (#29) + прод-деплой `maal-web-1` + go-live (флаг on).** Полный цикл в изолированном worktree: план (7 задач) → subagent-driven (имплементер `coder` + spec-ревью + code-quality-ревью на содержательные: адаптер/callback/UI; тривиальные inline) → финальный холистический ревью → merge → прод ship-dark → owner E2E → go-live.

- **Фича:** «Войти через Точку» на `/login`+`/register`, зеркалит Яндекс-callback. OAuth-юзер `email_confirm:true` (обходит недоставку DOI на `@tochka.com`). OIDC-клиент `apps/web/src/lib/auth/tochka.ts` портирован из go.mpstats (Variti-WAF cookie-handshake, Basic-auth token exchange). `TochkaProvider` адаптер (`OAuthUserInfo.email`→`string|null`+`emailVerified?/phoneVerified?`), server-action `signInWithTochka`, callback-роут (find/create по `lower(email)`, magiclink-сессия, phone, trial|referral как Яндекс, ветка `tochka_no_email`). Аддит. `UserProfile.tochkaId @unique` (Mgmt API, additive). Флаг `TOCHKA_LOGIN_ENABLED` + обе auth-страницы `force-dynamic` (рантайм-флип без ребилда). Логотип — фиолетовый бейдж «Точка Банк» (`public/tochka-badge.svg` из фирстиля).
- **🎯 Прод-находка (scope):** боевой клиент `245da9…` (общий с go.mpstats) принимает **ТОЛЬКО `scope=default`** — `openid`/`openid,customers`/`customers`/`accounts`/`openapi` → **400 «Incorrect scope»** (проверено пробой authorize через WAF-handshake). go.mpstats дефолтил `openid,customers` (невалидно для клиента → их вход, вероятно, не тестился E2E). Зашил `default` дефолтом (#29) + `TOCHKA_SCOPE=default` в прод-env. **`default` отдаёт email** (owner залогинился по совпадению email — иначе был бы `tochka_no_email`).
- **E2E owner (zebrosha@list.ru):** вход отработал — ветка **existing→линк** (email уже был в базе): сессия создана, `tochka_id`/`UserProfile.tochkaId` обновлены, онбординг пройден. Ветка **нового** юзера (`email_confirm:true`+триал+`/complete-profile`) на проде НЕ гонялась (email owner существовал) — покрыта mock-тестами + Yandex-parity. Tochka-юзеры → amoCRM как обычные лиды.
- **Деплой:** merge→master, прод `build --no-cache web`+recreate, env-block (креды из `go_mpstats/.env.local`, флаг+`TOCHKA_SCOPE=default`) в `.env.production` (бэкап `.bak.*`), dark-смоук (кнопка скрыта)→owner E2E→**go-live (флаг on)**. **Откат ~10с:** `sed s/=true/=false/ .env.production` + `docker compose up -d --force-recreate --no-build web`, либо `git revert -m 1 fce6470`.
- **CI-нюанс:** test-job красный = предсущ. флейки (`yandex-oauth` 5s-таймаут под нагрузкой + deprecated `question-bank`, оба зелёные локально, не в моих файлах); tochka-тесты 10+4 зелёные, typecheck+lint зелёные.
- **Follow-up:** юнит-тесты самого `tochka.ts` (WAF-хендшейк/error-пути); проверить ветку нового юзера на первом реальном новом Точка-юзере. Память `project_tochka_oauth_login_shipped.md`, гоча worktree-путей `feedback_worktree_absolute_paths.md`.

## Previous Session (2026-07-08) — Пост-онбординг «первый урок» + trial-gap фикс shipped to prod

Сессия началась с разбора жалобы (stepanova.forever@yandex.ru) + запроса owner на воронку: из 164 рег/3нед 138 проходят онбординг, но 23% открывают урок; 55% прошедших визард — ничего. Два релиза на прод.

**1. Trial gap fix (merge `3b5e6f1`).** `/auth/callback` (PKCE code-exchange путь DOI-подтверждения) не звал `ensureBaseTrial` (в отличие от `/auth/confirm` + yandex-callback) → ~46% подтверждённых юзеров без авто-триала = paywall как награда за онбординг. Зеркалил реф/триал-блок, гейт на recovery (`?next=/reset-password`). Бэкафилл 57 триалов «обделённым». Гоча: legacy `service_role` JWT отключён после утечки 2026-05-21 → Admin API только новым `sb_secret_*` (`MAAL/.env` `SUPABASE_SECRET_KEY`); память устарела.

**2. Пост-онбординг первый урок (merge `f9120da`).** Полный цикл brainstorm→spec→plan→subagent-driven (имплементер `coder` + ревью каждая + финальное холистическое)→staging→UAT-итерации→прод. Развилка `ForkScreen` удалена → `/dashboard` герой «Твой первый урок» (`resolveFirstLesson` цель×МП, мэппинг методологов); «Начни отсюда» визард-driven; диагностика снята со входа. Визуал по UAT-фидбэку (градиент-герой, приветствие без фона, порядок кнопок, pink). Жизненный цикл героя = по касанию. **Гоча:** модалка завершения только при `?from=` контексте (голый hero-вход авто-переходит на след. урок, модалка не рендерится) → наш after-value CTA откачен, оставлена пред-существующая `DiagnosticGateBanner`. **Клик-трекинга кнопок/разделов в БД НЕТ** (только ref-clicks + heartbeat + CQ). Тесты: api 403/403, typecheck 6/6, welcome 8/8. Детали + мэппинг lessonId: `project_post_onboarding_first_lesson.md`.

**Гоча сессии — параллельные инстансы:** другой Claude-инстанс создал `feature/tochka-oauth-login` из вершины моей in-flight onboarding-ветки (тащил весь мой неутверждённый onboarding + мой коммит улёгся на его ветку, т.к. HEAD переключился в общем дереве). Распутано БЕЗ правки его ветки: cherry-pick моего коммита на свою ветку через изолированный `git worktree` + рекомендация ребейзнуть tochka от master. Урок: параллельная работа = обязательно `git worktree`, не общее дерево.

## Previous Session (2026-07-03) — Ось-центричный редизайн диагностики/плана + оживший разбор ошибок shipped to prod
...
## Previous Session (2026-07-01) — Пересборка CTA главной + go-live партнёрской ручки MPSTATS
...
## Previous Session (2026-06-30) — 3 фикса на прод: телефоны лидов, ассистент урока, склонение плашек
