...
2. `pnpm --filter web build` локально (ловит server-only-в-client) + `pnpm typecheck` + тесты.
   **Общая БД (важно):** staging и prod ходят в один Supabase, поэтому состояние джобы
   общее. Решение: джоба **навсегда `isPublished=false`** (нет в обычном каталоге → ноль
   утечки на прод), гейт ЧП-поверхностей — **только per-env флаг**, а `getJob`/`getTitleBySlug`
   байпасят `isPublished` для ЧП-slug, чтобы детальная страница открывалась. Публиковать
   джобу не нужно ни на staging, ни на запуске.
3. **Staging**: `docker-compose.staging.yml`, `build --no-cache web`, флаг
   `EMERGENCY_BANNER_ENABLED=true` → **отсмотреть баннер, пин и детальную страницу на
   staging** (прод с флагом off не затронут). После — `git checkout master` до prod-деплоя.
4. **Prod, тёмный**: мерж в master, деплой, флаг `false` → **заметный ЧП-блок** (баннер +
   пин) не виден, джоба неопубликована → её нет и в обычном каталоге. Оговорка: сам
   урок-вебинар лежит в курсе «Практикумы» с `isHidden=false`, поэтому доступен в общей
...
5. **Запуск**: `EMERGENCY_BANNER_ENABLED=true` + `up -d web`. Баннер и пин зажигаются, без
   публикации джобы. Отсмотреть на проде.
