 ...
 *
 * Известные ограничения (без UA-базы моделей это неизбежные пограничные случаи):
 * - iPadOS 13+ по умолчанию представляется как Macintosh — такие заходы попадут в DESKTOP.
 * - Android-телефон с включённым «Запросить версию для ПК» теряет токен "Mobile" —
 *   такой UA попадёт в TABLET.
 * - Android-планшеты, которые всё же сохраняют токен "Mobile" (встречается на части
 *   старых или отдельных Samsung/Huawei прошивок) — попадут в MOBILE.
 * Лечится только client hints; для наших целей (доля мобильных vs десктоп) погрешность
 * приемлема.
 */
