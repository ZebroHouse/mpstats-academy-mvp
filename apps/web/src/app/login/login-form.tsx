...
import { signIn, signInWithYandex, signInWithTochka } from '@/lib/auth/actions';
...
function LoginForm({ tochkaEnabled }: { tochkaEnabled?: boolean }) {
  ...
  async function handleTochkaSignIn() {
    ...
    reachGoal(METRIKA_GOALS.LOGIN, { method: 'tochka' });
...
export function LoginPageClient({ tochkaEnabled }: { tochkaEnabled?: boolean }) {
  ...
  return (
    <div className="mx-auto flex min-h-[68vh] w-full max-w-md flex-col justify-center px-4 py-12 lg:py-16">
      <Suspense fallback={<div className="animate-pulse text-white/40">Загрузка...</div>}>
        <LoginForm tochkaEnabled={tochkaEnabled} />
      </Suspense>
    </div>
  );
}
