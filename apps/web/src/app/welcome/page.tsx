 ...
 * Client-side useState stepper (1 → 2 → 3). Answers accumulate locally; a single
 * onboarding.complete mutation persists everything when step 3 finishes. Everyone
 * lands on /dashboard (partner ?next= overrides), where a cold user gets the
 * «Твой первый урок» hero.
 *
 * Completion uses a hard load (window.location.assign), NOT router.push. The
 * (main) layout guard redirects to /welcome while onboardingCompletedAt is null;
 * after a soft navigation Next's client Router Cache can replay a stale
 * pre-onboarding render of that guard — bouncing the user back into the wizard
 * even though complete() already wrote the flag (prod incident 2026-05-19). A
 * full load discards the Router Cache so the guard re-renders server-side against
 * the freshly-written flag.
 */
