...
const navItems: NavItem[] = [
  ...
  {
    // MPSTATS logo mark = platform LogoIcon in the MPSTATS brand green (#17BF50).
    title: 'Инструменты',
    href: '/mpstats-tools',
    icon: <LogoIcon size={24} primaryColor="#17BF50" className="w-6 h-6" />,
  },
  {
    title: 'Профиль',
    href: '/profile',
...
export function MobileNav({ partnerEnabled = false }: { partnerEnabled?: boolean }) {
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-lg transition-all duration-200 min-w-0 shrink-0',
                isActive
                  ? 'text-mp-blue-600'
                  : 'text-mp-gray-400 active:text-mp-gray-600'
              )}
            >
              <span className={cn(
              ...
              <span className={cn(
                ...
                isActive && 'text-mp-blue-600'
              )}>
                {item.title}
              </span>
              {isActive && (
                <span className="absolute bottom-0 w-8 h-0.5 bg-mp-blue-500 rounded-full" />
              )}
            </Link>
          );
        })}
